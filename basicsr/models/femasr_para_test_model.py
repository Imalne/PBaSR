...
class FeMaSRParaTestModel(BaseModel):
    def __init__(self, opt):
        ...
        hq_opt = self.opt['network_g']['general_branch_parameter'].copy()
    ...
    def feed_data(self, data):
        if not isinstance(data, dict):
            ...
            # blur data
            self.blur_lq = blur_data['lq'].to(self.device)
            if 'gt' in blur_data:
                self.blur_gt = blur_data['gt'].to(self.device)
            if 'blur_mask' in blur_data:
                self.blur_blur_mask = blur_data['blur_mask'].to(self.device)
        else:
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger,
                           save_img, save_as_dir, img_save_max_num=100):
    ...
    def get_current_visuals(self):
        ...
        out_dict['blur_lq'] = self.blur_lq.detach().cpu()[:vis_samples]
        out_dict['blur_result'] = self.blur_output.detach().cpu()[:vis_samples]
        if hasattr(self, 'blur_gt_rec'):
            out_dict['blur_gt_rec'] = self.blur_gt_rec.detach().cpu()[:vis_samples]
        if hasattr(self, 'blur_gt'):
            out_dict['blur_gt'] = self.blur_gt.detach().cpu()[:vis_samples]
