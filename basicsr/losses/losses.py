...
@LOSS_REGISTRY.register()
class MaskedLPIPSLoss(nn.Module):
    """LPIPS loss with vgg backbone.
    """
    def __init__(self, loss_weight = 1.0):
        super(MaskedLPIPSLoss, self).__init__()
        self.model = pyiqa.create_metric('lpips-vgg', as_loss=True,use_mask=True)
        self.loss_weight = loss_weight

    def forward(self, x, gt, **kwargs):
        return self.model(x, gt, **kwargs) * self.loss_weight, None
...
class MaskedGANLoss(nn.Module):
    ...
    def __init__(self, gan_type, real_label_val=1.0, fake_label_val=0.0, loss_weight=1.0):
        super(MaskedGANLoss, self).__init__()
        self.gan_type = gan_type
        self.loss_weight = loss_weight
        self.real_label_val = real_label_val
        self.fake_label_val = fake_label_val
