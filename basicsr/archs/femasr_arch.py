...
class FeMaSRNet(nn.Module):
    ...
    def encode_and_decode(self, input, gt_indices=None, current_iter=None, rtn_feats=False):
        ...
        if rtn_feats:
            return out_img, codebook_loss, semantic_loss, indices_list, enc_feats
        else:
    ...
    @torch.no_grad()
    def test_tile(self, input, tile_size=240, tile_pad=16, rtn_all=False):
        ...
        for y in range(tiles_y):
            for x in range(tiles_x):
                ...
                # upscale tile
                output_tile, b, c, _ = self.test(input_tile, rtn_all=True)
                b_s.append(b)
                c_s.append(c)
        ...
        if rtn_all:
            return output, torch.mean(torch.stack(b_s)), torch.mean(torch.stack(c_s))
        else:
            return output

    @torch.no_grad()
    def test(self, input, rtn_all=False):
        ...
        dec, codebook_loss, semantic_loss, indices_list = self.encode_and_decode(input)
        ...
        if not rtn_all:
            ...
            return output, codebook_loss, semantic_loss, indices_list
